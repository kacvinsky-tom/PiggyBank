package cz.muni.fi.pv168.piggybank.data;

import cz.muni.fi.pv168.piggybank.enums.TransactionType;
import cz.muni.fi.pv168.piggybank.model.Category;
import cz.muni.fi.pv168.piggybank.model.CategoryStatistic;
...
public class StatisticDao {
    ...
    public int getNumberOfTransactions(Category category, Date from, Date to){
        ...
        try (var connection = dataSource.getConnection();
            ...
            try (var rs = st.executeQuery()) {
                while (rs.next()) {
                    Date creationDate = new Date(rs.getDate("CREATION_DATE").getTime());
